 ...
 */
@Entity
public class BestSellerBanner extends BaseModel{
    private static final long serialVersionUID = 1L;
    public static final boolean ACTIVE = true;
    public static final boolean INACTIVE = false;
    private static final String LOG_TYPE = "ADMIN";
    private static final String LOG_TABLE_NAME = "best_seller_banner";
    ...
    public static Finder<Long, BestSellerBanner> find = new Finder<Long, BestSellerBanner>(Long.class, BestSellerBanner.class);

    public static String validation(BestSellerBanner model){
    ...
    public static int getNextSequence(Long brandId){
        SqlQuery sqlQuery = Ebean.createSqlQuery(
                "select max(sequence) as max from best_seller_banner where status = true and brand_id = :brandId");
    ...
    public static Page<BestSellerBanner> page(int page, int pageSize, String sortBy, String order, String name, Integer filter, Brand brand) {
        ExpressionList<BestSellerBanner> qry = BestSellerBanner.find
                .where()
                .ilike("name", "%" + name + "%")
                .eq("is_deleted", false)
                .eq("brand", brand);
    ...
    public String getChangeLogData(BestSellerBanner data){
    ...
    @Override
    public void update() {
        BestSellerBanner oldBanner = BestSellerBanner.find.byId(id);
        super.update();
    ...
    public void updateStatus(String newStatus) {
        ...
        if(newStatus.equals("active"))
            status = BestSellerBanner.ACTIVE;
        else if(newStatus.equals("inactive"))
            status = BestSellerBanner.INACTIVE;
    ...
    public static BestSellerBanner findById(Long id, Long brandId) {
        return find.where().eq("is_deleted", false).eq("id", id).eq("brand_id", brandId).findUnique();
    }

    public static List<BestSellerBanner> getAllBanner(Long brandId, Integer limit) {
        Date now = new Date();
        return find
                .where()
//                .le("active_from", now)
...
//                .eq("status", true)
    ...
    public static BannerList getDetails(String slug, Long brandId){
        Date now = new Date();
        BestSellerBanner banner = find.where()
//                .eq("status", true)
                .eq("is_deleted", false)
                .eq("slug", slug)
                .eq("brand_id", brandId)
