...
public class BestSeller extends BaseModel{
    ...
    private static final String LOG_TABLE_NAME = "best_seller";
    ...
    public static Finder<Long, BestSeller> find = new Finder<Long, BestSeller>(Long.class, BestSeller.class);

    public static int getNextSequence(Long brandId){
        SqlQuery sqlQuery = Ebean.createSqlQuery(
                "select max(sequence) as max from best_seller where status = true and brand_id = :brandId");
    ...
    public static Page<BestSeller> page(int page, int pageSize, String sortBy, String order, String name, Integer filter, Brand brand) {
        ExpressionList<BestSeller> qry = BestSeller.find
                .where()
    ...
    public static BestSeller findByProduct(Product product, Brand brand) {
        return
                find.where()
                        .eq("is_deleted", false)
                        .eq("brand", brand)
    ...
    public String getChangeLogData(BestSeller data){
    ...
    @Override
    public void update() {
        BestSeller oldBanner = BestSeller.find.byId(id);
        super.update();
    ...
    public void updateStatus(String newStatus) {
        ...
        if(newStatus.equals("active"))
            status = BestSeller.ACTIVE;
        else if(newStatus.equals("inactive"))
            status = BestSeller.INACTIVE;
    ...
    public static BestSeller findById(Long id, Long brandId) {
    ...
    public static List<Product> getAllData(Long brandId, Integer limit) {
        List<BestSeller> data = find.where()
                .eq("is_deleted", false)
                .eq("brand_id", brandId)
                .eq("status", true)
                .setMaxRows(limit)
