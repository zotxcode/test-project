 ...
 */
@Entity
public class InstagramBanner extends BaseModel{
    private static final long serialVersionUID = 1L;
    public static final boolean ACTIVE = true;
    public static final boolean INACTIVE = false;
    private static final String LOG_TYPE = "ADMIN";
    private static final String LOG_TABLE_NAME = "instagram_banner";
    ...
    public static Finder<Long, InstagramBanner> find = new Finder<Long, InstagramBanner>(Long.class, InstagramBanner.class);

    public static String validation(InstagramBanner model){
    ...
    public static int getNextSequence(Long brandId){
        SqlQuery sqlQuery = Ebean.createSqlQuery(
                "select max(sequence) as max from instagram_banner where status = true and brand_id = :brandId");
    ...
    public static Page<InstagramBanner> page(int page, int pageSize, String sortBy, String order, String name, Integer filter, Brand brand) {
        ExpressionList<InstagramBanner> qry = InstagramBanner.find
                .where()
                .ilike("name", "%" + name + "%")
                .eq("is_deleted", false)
                .eq("brand", brand);
    ...
    public String getChangeLogData(InstagramBanner data){
    ...
    @Override
    public void update() {
        InstagramBanner oldBanner = InstagramBanner.find.byId(id);
        super.update();
    ...
    public void updateStatus(String newStatus) {
        ...
        if(newStatus.equals("active"))
            status = InstagramBanner.ACTIVE;
        else if(newStatus.equals("inactive"))
            status = InstagramBanner.INACTIVE;
    ...
    public static InstagramBanner findById(Long id, Long brandId) {
        return find.where().eq("is_deleted", false).eq("id", id).eq("brand_id", brandId).findUnique();
    }

    public static List<InstagramBanner> getAllBanner(Long brandId, Integer limit) {
        Date now = new Date();
        return find
                .where()
//                .le("active_from", now)
...
//                .eq("status", true)
    ...
    public static BannerList getDetails(String slug, Long brandId){
        Date now = new Date();
        InstagramBanner banner = find.where()
//                .eq("status", true)
                .eq("is_deleted", false)
                .eq("slug", slug)
                .eq("brand_id", brandId)
