...
public class CategoryBanner extends BaseModel{
    ...
    public static Finder<Long, CategoryBanner> find = new Finder<Long, CategoryBanner>(Long.class, CategoryBanner.class);
    ...
    public static int getNextSequence(Long brandId){
        SqlQuery sqlQuery = Ebean.createSqlQuery(
                "select max(sequence) as max from category_banner where status = true and brand_id = :brandId");
    ...
    public static Page<CategoryBanner> page(int page, int pageSize, String sortBy, String order, String name, Integer filter, Brand brand) {
    ...
    @Override
    public void update() {
        CategoryBanner oldBanner = CategoryBanner.find.byId(id);
        super.update();
    ...
    public static CategoryBanner findById(Long id, Long brandId) {
        return find.where().eq("is_deleted", false).eq("id", id).eq("brand_id", brandId).findUnique();
    }

    public static List<CategoryBanner> getAllBanner(Long brandId, Integer limit) {
        Date now = new Date();
        return find
                .where()
//                .le("active_from", now)
...
//                .eq("status", true)
    ...
    public static BannerList getDetails(String slug, Long brandId){
        Date now = new Date();
        CategoryBanner banner = find.where()
//                .eq("status", true)
                .eq("is_deleted", false)
                .eq("slug", slug)
                .eq("brand_id", brandId)
