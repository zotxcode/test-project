 ...
 */
@Entity
public class OnSaleBanner extends BaseModel{
    private static final long serialVersionUID = 1L;
    public static final boolean ACTIVE = true;
    public static final boolean INACTIVE = false;
    private static final String LOG_TYPE = "ADMIN";
    private static final String LOG_TABLE_NAME = "on_sale_banner";
...
//    public String externalUrl;
    ...
    public static Finder<Long, OnSaleBanner> find = new Finder<Long, OnSaleBanner>(Long.class, OnSaleBanner.class);

    public static String validation(OnSaleBanner model){
    ...
    public static int getNextSequence(Long brandId){
        SqlQuery sqlQuery = Ebean.createSqlQuery(
                "select max(sequence) as max from on_sale_banner where status = true and brand_id = :brandId");
    ...
    public static Page<OnSaleBanner> page(int page, int pageSize, String sortBy, String order, String name, Integer filter, Brand brand) {
        ExpressionList<OnSaleBanner> qry = OnSaleBanner.find
                .where()
                .ilike("name", "%" + name + "%")
                .eq("is_deleted", false)
                .eq("brand", brand);
    ...
    public String getChangeLogData(OnSaleBanner data){
    ...
    @Override
    public void update() {
        OnSaleBanner oldBanner = OnSaleBanner.find.byId(id);
        super.update();
    ...
    public void updateStatus(String newStatus) {
        ...
        if(newStatus.equals("active"))
            status = OnSaleBanner.ACTIVE;
        else if(newStatus.equals("inactive"))
            status = OnSaleBanner.INACTIVE;
    ...
    public static OnSaleBanner findById(Long id, Long brandId) {
        return find.where().eq("is_deleted", false).eq("id", id).eq("brand_id", brandId).findUnique();
    }

    public static List<OnSaleBanner> getAllBanner(Long brandId, Integer limit) {
        Date now = new Date();
        return find
                .where()
//                .le("active_from", now)
...
//                .eq("status", true)
    ...
    public static BannerList getDetails(String slug, Long brandId){
        Date now = new Date();
        OnSaleBanner banner = find.where()
//                .eq("status", true)
                .eq("is_deleted", false)
                .eq("slug", slug)
                .eq("brand_id", brandId)
