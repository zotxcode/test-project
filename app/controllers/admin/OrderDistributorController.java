...
import views.html.admin.orderdistributor.detail;
import views.html.admin.orderdistributor.list;
...
public class OrderDistributorController extends BaseController {
    private static final String TITLE = "Order Distributor";
    private static final String featureKey = "orderdistributor";
    ...
    public static Result lists() {
        ...
        Page<PurchaseOrder> datas = PurchaseOrder.page(page, pageSize, sortBy, order, name, null);
        ...
        for (PurchaseOrder dt : datas.getList()) {
            ...
            String action = "";
            if (feature.isView()){
                action += "<a class=\"btn btn-default btn-sm action\" title=\"Detail\" href=\"" + routes.OrderDistributorController.detail(dt.id) + "\"><i class=\"fa fa-search\"></i></a>&nbsp;";
            }
            if(feature.isEdit()){
                switch (dt.status) {
                    case 1:
            ...
            row.put("0", num);
            row.put("1", dt.code);
            row.put("2", dt.distributor.fullName);
        ...
        try {
            ...
            for (int i=0; i < tmp.length; i++){
                ...
                for (PurchaseOrderDetail pd : data.details){
                    ...
                    if (ps == null){
                        ps = new ProductStock();
                        ps.distributor = data.distributor;
                        ps.product = pd.product;
                        ps.stock = pd.qty.longValue();
                        ps.save();
                    }else {
