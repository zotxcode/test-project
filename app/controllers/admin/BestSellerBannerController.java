...
import views.html.admin.bestSellerBanner._form;
import views.html.admin.bestSellerBanner.list;
import views.html.admin.bestSellerBanner.sequence;
 ...
 */
public class BestSellerBannerController extends BaseController {
    private final static Logger.ALogger logger = Logger.of(BestSellerBannerController.class);
    private static final String TITLE = "Best Seller Banner";
    private static final String featureKey = "best_seller_banner";
    ...
    private static Html htmlEdit(Form<BestSellerBanner> data, Map<Integer, String> listsCategory, Map<Integer, String> listsSubCategory, Map<Integer, String> listsProduct){
        Integer width, height, width2, height2;
        if (Constant.getInstance().isEversac()){
            width = 1138;
            height = 450;
            height2 = 200;
            width2 = 569;
        ...
        return _form.render(TITLE, "Edit", data, routes.BestSellerBannerController.update(), listsCategory, listsSubCategory, listsProduct, width, height, width2, height2, getType());
    }

    private static Html htmlAdd(Form<BestSellerBanner> data){
        Integer width, height, width2, height2;
        if (Constant.getInstance().isEversac()){
            width = 1138;
            height = 450;
        ...
        return _form.render(TITLE, "Add", data, routes.BestSellerBannerController.save(), new LinkedHashMap<>(), new LinkedHashMap<>(), new LinkedHashMap<>(), width, height, width2, height2, getType());
    }
    ...
    private static List<BestSellerBanner> getListBannerByType() {
        Query<BestSellerBanner> qry = BestSellerBanner.find.where()
                .eq("is_deleted", false)
                .eq("brand", getBrand())
                .eq("status", true)
    ...
    public static Result add() {
        BestSellerBanner dt = new BestSellerBanner();
        dt.fromDate = Helper.getDateFromTimeStamp(new Date());
        Date toDate = new Date();
        toDate.setDate(toDate.getDate()+1);
        dt.toDate = Helper.getDateFromTimeStamp(toDate);
        Form<BestSellerBanner> formData = Form.form(BestSellerBanner.class).fill(dt);
        return ok(htmlAdd(formData));
    }

    @Security.Authenticated(Secured.class)
    public static Result edit(Long id) {
        BestSellerBanner dt = BestSellerBanner.findById(id, getBrandId());
    ...
    public static Result lists() {
        ...
        Integer iTotalRecords = BestSellerBanner.findRowCount(getBrand());
        ...
        Page<BestSellerBanner> datas = BestSellerBanner.page(page, pageSize, sortBy, order, name, filter, getBrand());
        ...
        for (BestSellerBanner dt : datas.getList()) {
            ObjectNode row = Json.newObject();
            String action = "" ;
            if(feature.isEdit()){
                action += "<a class=\"btn btn-primary btn-sm action\" href=\""+ routes.BestSellerBannerController.edit(dt.id)+"\"><i class=\"fa fa-pencil\"></i></a>&nbsp;" ;
            }
            if(feature.isDelete()){
    ...
    private static int[] getResolution(){
        return Constant.getInstance().isEversac() ? Photo.bestSellerSizeEversac : Photo.bestSellerSize;
    }

    private static int[] getResolutionResponsive(){
        return Constant.getInstance().isEversac() ? Photo.bestSellerEversacResponsiveSize : Photo.bestSellerResponsiveSize;
    }

    @Security.Authenticated(Secured.class)
    public static Result save() {
        Form<BestSellerBanner> form = Form.form(BestSellerBanner.class).bindFromRequest();
        ...
        if (form.hasErrors()){
            ...
            BestSellerBanner data = form.get();
            ...
            try {
                ...
                newFile = Photo.uploadImageCrop2(picture, "bs-ban", imageName, data.imageUrlX, data.imageUrlY, data.imageUrlW, data.imageUrlH, getResolution(), "jpg");

                newFileRes = Photo.uploadImageCrop2(pictureResponsive, "bs-ban-res", imageName,
                        data.imageUrlResponsiveX, data.imageUrlResponsiveY, data.imageUrlResponsiveW, data.imageUrlResponsiveH, getResolutionResponsive(), "jpg");
                data.sequence = BestSellerBanner.getNextSequence(getBrandId());

                UserCms cms = getUserCms();
                data.imageUrl = (newFile == null) ? data.imageUrl
                        : Photo.createUrl("bs-ban", newFile.getName());
                data.imageUrlResponsive = (newFileRes == null) ? data.imageUrlResponsive
                        : Photo.createUrl("bs-ban-res", newFileRes.getName());
                ...
                if (newFile != null) {
                    Photo.saveRecord("bs-ban", newFile.getName(), "", "", "", picture.getFilename(), cms.id, roleKey,
                            "Banner", data.id);
                }

                if (newFileRes != null) {
                    Photo.saveRecord("bs-ban-res", newFileRes.getName(), "", "", "", pictureResponsive.getFilename(), cms.id,
            ...
            if (data.save.equals("1")){
                return redirect(routes.BestSellerBannerController.index());
            }else{
                return redirect(routes.BestSellerBannerController.add());
            }
    ...
    public static Result update() {
        ...
        if (form.hasErrors()){
            ...
            try {
                ...
                String roleKey = cms.role.key;
                if (newFile != null){
                    data.imageUrl = Photo.createUrl("bs-ban", newFile.getName());
                    Photo.saveRecord("bs-ban", newFile.getName(), "", "", "", picture.getFilename(), cms.id, roleKey,
                            "Banner", data.id);
                }
                if (newFileRes != null) {
                    data.imageUrlResponsive = Photo.createUrl("bs-ban-res", newFileRes.getName());
                    Photo.saveRecord("bs-ban-res", newFileRes.getName(), "", "", "", pictureResponsive.getFilename(), cms.id,
                            roleKey, "Banner", data.id);
                }
    ...
    public static Result updateStatus(String id, String newStatus) {
        ...
        try {
            String[] tmp = id.split(",");
            for (String aTmp : tmp) {
                BestSellerBanner data = BestSellerBanner.findById(Long.parseLong(aTmp), getBrandId());
    ...
    public static Result updateSequence() {
        ...
        try {
            sequences.forEach((k,v)->{
                int loop = 0;
                for (Integer dt : v) {
                    BestSellerBanner data = BestSellerBanner.findById(dt.longValue(), getBrandId());
